import 'dart:convert';

import 'package:delivery/controller/bev_main_controller.dart';
import 'package:delivery/controller/institution_controller.dart';

import 'package:delivery/model/menu_model.dart';
import 'package:get_it/get_it.dart';
import 'package:http/http.dart' as http;
import 'package:mobx/mobx.dart';

import '../constants.dart';
part 'bev_items_controller.g.dart';

class BevItemsController = _BevItemsController with _$BevItemsController;

abstract class _BevItemsController with Store {
  final String _baseUrl = '${Constants.BASE_API_URL}/RestButton/getlist';

  @observable
  String menuDesc = 'Medida';

  List<MenuModel> menuModel = [];
  final institutionController = GetIt.I.get<InstitutionController>();
  final bevMainController = GetIt.I.get<BevMainController>();
  Future getPosts(String url) async {
    final response = await http.post(
      _baseUrl,
      headers: <String, String>{
        'Content-Type': 'application/json; charset=UTF-8',
      },
      body: jsonEncode(<String, String>{
        'tb_institution_id': "${institutionController.institution.id}",
        'id': "${bevMainController.chosenMenuId}",
      }),
    );

    if (response.statusCode == 200) {
      final data = jsonDecode(response.body);
      menuModel = (data as List).map((json) {
        return MenuModel(
          id: json['id'],
          description: json["description"],
          priceTag: double.parse(json['price_tag']),
          measure: json["measure"],
          linkUrl: json['link_url'],
        );
      }).toList();
      return menuModel;
    } else {
      print("Error in URL");
    }
  }

  @observable
  ObservableFuture<List<MenuModel>> postsListFuture;
  @observable
  List<MenuModel> listFiltered;

  @action
  filter(List<MenuModel> list, String search) {
    if (search != '') {
      listFiltered = list
          .where((f) =>
              (f.description.toUpperCase().contains(search.toUpperCase())))
          .toList();
    } else {
      listFiltered = list;
    }
  }

  @action
  Future fetchPosts() => postsListFuture =
      ObservableFuture(getPosts(_baseUrl).then((posts) => posts));

  void getThePosts() {
    fetchPosts();
  }
}
